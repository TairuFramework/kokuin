...
describe('generateKeyRecord', () => {
  test('produces a non-extractable signing key and a raw agreement secret', async () => {
    const record = await generateKeyRecord()
    expect(record.suite).toBe('Ed25519')
    expect(record.signing.extractable).toBe(false)
    expect(record.agreementSecret).toBeInstanceOf(Uint8Array)
  ...
  test('the agreement secret is the one a sender derives from the DID — not an independent key', async () => {
    const record = await generateKeyRecord()

    // A sender knows only the Ed25519 public key (from the did:key DID) and derives the
    // agreement key from it. If the agreement secret were independent, this ECDH would not
    // agree and nothing addressed to the DID could ever be decrypted.
    ...
    const ourShared = x25519.getSharedSecret(record.agreementSecret, senderPublic)
